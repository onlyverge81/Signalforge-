...
import { fetchPolygonAggs, aggregate } from "./pattern-study.mjs";
...
async function main(){
  ...
  for(let i = 0; i < syms.length; i++){
    const sym = syms[i];
    try {
      const candles = await fetchPolygonAggs(sym, resolution, key, { lookbackMs: lookbackDays * 864e5, minBars: 120 });
      const audit = auditData(candles);
      if(audit.suspect){ skipped.push({ sym, reason: "data audit" }); continue; }
      withData++;
      for(const h of horizons){
        const bt = backtestPattern(candles, { horizon: h, trendFilter: true });
  ...
  const out = {
    ...
    priceSrc: `Polygon ${resolution} (adjusted)`,
