...
function closed({ id = "T", ticker = "X", signal = "BUY", entry = 100, exit, benchClose, benchDiv, grade = null, merits = false, momentum = false, reversal = false, lowVol = false, quality = false, newsPositive = false, newsQuiet = true, earningsRecent = false, contenderAllBoxes = false, contenderAB = false, contenderC = false, contenderDF = false, convergence = false, esdHeading = false, breadthQuorum = false, breadthVolConfirmed = false }) {
  ...
  return {
    ...
    benchClose, benchDiv,
    tags: { fundamentalGrade: grade, meritsActivated: merits, momentumActivated: momentum, reversalActivated: reversal, lowVolActivated: lowVol, qualityActivated: quality, newsPositive, newsQuiet, earningsRecent, contenderAllBoxes, contenderAB, contenderC, contenderDF, convergence, esdHeading, breadthQuorum, breadthVolConfirmed },
  };
}
...
test("scoreLedger: contenders-on / contenders-off partition the SAME population by the contenderAllBoxes tag", () => {
  const ledger = [
    closed({ id: "A", entry: 100, exit: 107, benchClose: 102, contenderAllBoxes: true,  contenderAB: true }),  // shortlisted, +alpha
    closed({ id: "B", entry: 100, exit: 101, benchClose: 110, contenderAllBoxes: false, contenderAB: false }), // not, -alpha
    closed({ id: "C", entry: 100, exit: 109, benchClose: 103, contenderAllBoxes: true,  contenderAB: true }),  // shortlisted, +alpha
  ];
  const perf = scoreLedger(ledger);
  assert.equal(perf.variants["contenders-on"].n + perf.variants["contenders-off"].n, perf.variants["all"].n);
  assert.equal(perf.variants["contenders-on"].n, 2);
  assert.equal(perf.variants["contenders-off"].n, 1);
...
test("scoreLedger: contender-c-on/off and contender-df-on/off partition the tactical population; -on legs are FDR hypotheses", () => {
  const ledger = [
    closed({ id: "A", entry: 100, exit: 107, benchClose: 102, contenderC: true }),   // C watch tier
    closed({ id: "B", entry: 100, exit: 101, benchClose: 110, contenderDF: true }),   // D/F low tier
    closed({ id: "C", entry: 100, exit: 109, benchClose: 103 }),                      // neither tier
  ];
  const perf = scoreLedger(ledger);
  assert.equal(perf.variants["contender-c-on"].n + perf.variants["contender-c-off"].n, perf.variants["all"].n);
  assert.equal(perf.variants["contender-df-on"].n + perf.variants["contender-df-off"].n, perf.variants["all"].n);
  assert.equal(perf.variants["contender-c-on"].n, 1);
  assert.equal(perf.variants["contender-df-on"].n, 1);
  // Every grade tier is now OOS-eligible: the -on legs are genuine FDR promotion hypotheses; the
  // -off complement controls are excluded from the family denominator (R2).
  assert.equal(perf.variants["contender-c-on"].fdr, true);
  assert.equal(perf.variants["contender-df-on"].fdr, true);
  assert.equal(perf.variants["contender-c-off"].fdr, false);
  assert.equal(perf.variants["contender-df-off"].fdr, false);
});
...
test("scoreLedger: esd-grounded-on requires BOTH the below→up heading AND a grounded all-boxes name; -off is the complement; -on is an FDR hypothesis", () => {
  const ledger = [
    closed({ id: "A", entry: 100, exit: 108, benchClose: 102, esdHeading: true,  contenderAllBoxes: true  }), // heading AND grounded → ON
    closed({ id: "B", entry: 100, exit: 109, benchClose: 103, esdHeading: true,  contenderAllBoxes: false }), // heading but NOT grounded → OFF
    closed({ id: "C", entry: 100, exit: 107, benchClose: 104, esdHeading: false, contenderAllBoxes: true  }), // grounded but no heading → OFF
    closed({ id: "D", entry: 100, exit: 101, benchClose: 110, esdHeading: false, contenderAllBoxes: false }), // neither → OFF
  ];
  const perf = scoreLedger(ledger);
  assert.equal(perf.variants["esd-grounded-on"].n, 1, "only the heading × grounded row is ON");
  assert.equal(perf.variants["esd-grounded-on"].n + perf.variants["esd-grounded-off"].n, perf.variants["all"].n, "on/off re-union to the tactical population");
  assert.equal(perf.variants["esd-grounded-on"].fdr, true, "the -on hypothesis is in the FDR family");
  assert.equal(perf.variants["esd-grounded-off"].fdr, false, "the -off complement is a control, excluded from the FDR denominator");
});
